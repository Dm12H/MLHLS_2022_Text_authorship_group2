...
ENV VENV_PATH=$PROJECT_PATH/venv \
    MODEL_PATH=$PROJECT_PATH/text_authorship \
    APP_PATH=$PROJECT_PATH/app
RUN mkdir -p $APP_PATH && mkdir -p $MODEL_PATH && touch $PROJECT_PATH/logerrors.log
COPY text_authorship/ $MODEL_PATH
COPY app/ $APP_PATH
COPY bert_pretrained/ $PROJECT_PATH/bert_pretrained
COPY main.py train_model.py prepare_dataset.py tastack_deploy.pkl logreg.pkl settings.yml $PROJECT_PATH/
...
RUN python3 -m pip install --no-cache-dir -r $MODEL_PATH/ta_model/requirements.txt
